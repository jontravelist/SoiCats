...
--    corroboration from another open flag of the same type on the same dog
--  - award the original flagger +30 points on verification
--  - dogs_needing_help RPC for the 'Needs help' pinned section on the Feed
...
declare
  ...
  next_status    dog_status;
begin
  select * into flag from public.dog_health_flags where id = target_flag;
  ...
  if flagger_role in ('feeder', 'app_admin') then
    ...
    select count(*) into corroborating
      from public.dog_health_flags
     where dog_id = flag.dog_id
  ...
  next_status := case flag.flag_type
    when 'deceased' then 'deceased'::dog_status
    when 'missing'  then 'missing'::dog_status
    else                 'injured'::dog_status
  end;
  ...
  values (flag.flagged_by, 'flag_verified', 30, flag.id, 'dog_health_flag');
  ...
  insert into public.points_log (user_id, action_type, points, related_entity_id, related_entity_type)
  select flagged_by, 'flag_verified', 30, id, 'dog_health_flag' from promoted;
end;
$$;

create or replace function public.dog_health_flags_after_insert()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists dog_health_flags_auto_verify on public.dog_health_flags;
create trigger dog_health_flags_auto_verify
  after insert on public.dog_health_flags
  for each row execute function public.dog_health_flags_after_insert();

-- Dogs with a currently-verified, unresolved flag — what powers the
-- 'Needs help' pinned section. Returns the most recent verified flag per dog.
create or replace function public.dogs_needing_help(
...
as $$
  ...
  latest_flag as (
    select distinct on (f.dog_id)
      f.id, f.dog_id, f.flag_type, f.created_at, f.description
    from public.dog_health_flags f
    where f.status = 'verified'
      and f.resolved_at is null
    order by f.dog_id, f.created_at desc
  )
  select
    c.id,
    c.name,
  ...
  from latest_flag lf
  join public.dogs c on c.id = lf.dog_id
...
-- For the dog profile: latest open/verified flag (so we can show 'Reported
-- injured' or similar without a separate request).
create or replace function public.latest_flag_for_dog(target_dog uuid)
returns table (
  id          uuid,
  flag_type   flag_type,
  status      flag_status,
